...
import util.FacebookOAuthService;

@WebServlet(urlPatterns = {"/oauth/facebook", "/oauth2callback/facebook"})
public class FacebookOAuthServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        } else if ("/oauth2callback/facebook".equals(path)) {
    ...
    /**
     * Initiates the OAuth flow by redirecting to Facebook's authorization page
     */
    private void initiateOAuthFlow(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        String state = FacebookOAuthService.generateState();
        ...
        String authUrl = FacebookOAuthConfig.buildAuthorizationUrl(state);
        resp.sendRedirect(authUrl);
    }
    
    /**
     * Handles the callback from Facebook after authorization
     */
    private void handleOAuthCallback(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        String storedState = (String) session.getAttribute("facebook_oauth_state");
        ...
        if (code == null || code.isEmpty()) {
            req.setAttribute("error", "Authorization code not received from Facebook.");
            req.getRequestDispatcher("/login.jsp").forward(req, resp);
            return;
        }
        ...
        try {
            ...
            String accessToken = FacebookOAuthService.exchangeCodeForToken(code);
            
            // Get user information using the access token
            Map<String, String> userInfo = FacebookOAuthService.getUserInfo(accessToken);
            ...
            if (email == null || email.isEmpty()) {
                req.setAttribute("error", "Email not received from Facebook.");
                req.getRequestDispatcher("/login.jsp").forward(req, resp);
                return;
            }
            ...
            e.printStackTrace();
            req.setAttribute("error", "Error during Facebook OAuth authentication: " + e.getMessage());
            req.getRequestDispatcher("/login.jsp").forward(req, resp);
        }
    }
}